...
-removed([{set_attribute,3,"use sam_erl_anno:set_line/2 instead"},
          {attributes_info,'_',
           "sam_erl_anno:{column,line,location,text}/1 instead"},
          {token_info,'_',
           "erl_scan:{category,column,line,location,symbol,text}/1 instead"}]).

-removed_type([{column,0,"use sam_erl_anno:column() instead"},
               {line,0,"use sam_erl_anno:line() instead"},
               {location,0,"use sam_erl_anno:location() instead"}]).
...
-type token() :: {category(), Anno :: sam_erl_anno:anno(), symbol()}
               | {category(), Anno :: sam_erl_anno:anno()}.
-type tokens() :: [token()].
-type error_description() :: term().
-type error_info() :: {sam_erl_anno:location(), module(), error_description()}.
...
-spec string(String) -> Return when
      ...
      EndLocation :: sam_erl_anno:location(),
      ErrorLocation :: sam_erl_anno:location().
string(String) ->
    string(String, 1, []).
...
-spec string(String, StartLocation) -> Return when
      ...
      Return :: {'ok', Tokens :: tokens(), EndLocation}
              | {'error', ErrorInfo :: error_info(), ErrorLocation},
      StartLocation :: sam_erl_anno:location(),
      EndLocation :: sam_erl_anno:location(),
      ErrorLocation :: sam_erl_anno:location().
...
-type cont_fun() :: fun((char_spec(), #erl_scan{},
                         sam_erl_anno:line(), sam_erl_anno:column(),
                         tokens(), any()) -> any()).
-opaque return_cont() :: {erl_scan_continuation,
                          string(), sam_erl_anno:column(), tokens(),
                          sam_erl_anno:line(),
                          #erl_scan{}, any(), cont_fun()}.
-type tokens_result() :: {'ok', Tokens :: tokens(),
                          EndLocation :: sam_erl_anno:location()}
                       | {'eof', EndLocation :: sam_erl_anno:location()}
...
-spec column(Token) -> sam_erl_anno:column() | 'undefined' when
      Token :: token().

column(Token) ->
    sam_erl_anno:column(element(2, Token)).

-spec end_location(Token) -> sam_erl_anno:location() | 'undefined' when
      Token :: token().

end_location(Token) ->
    sam_erl_anno:end_location(element(2, Token)).

-spec line(Token) -> sam_erl_anno:line() when
      Token :: token().

line(Token) ->
    sam_erl_anno:line(element(2, Token)).

-spec location(Token) -> sam_erl_anno:location() when
      Token :: token().

location(Token) ->
    sam_erl_anno:location(element(2, Token)).

-spec text(Token) -> sam_erl_anno:text() | 'undefined' when
      Token :: token().

text(Token) ->
    sam_erl_anno:text(element(2, Token)).
...
anno(Line, no_col, #erl_scan{text = true}, String) ->
    Anno = anno(Line),
    sam_erl_anno:set_text(String, Anno);
